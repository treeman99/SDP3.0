...
[Step 1] Figma Reader Agent 실행
  ...
  │
  ▼
[Step 2] Builder Agent 실행                    ←── 코드 반복 시작점
  ...
  │
  ▼
[Step 3] Verifier Agent 실행 (코드 검증)
  │  agents/verifier.md 참조
  │  ⚠️ 반드시 사용자에게 브라우저 캡처 스크린샷을 요청한다
...
[Step 4] Architect Agent 실행
  ...
  │
  ▼
[Step 5] Verifier Agent 실행 (리팩토링 검증)
  │  agents/verifier.md 참조
  │  ⚠️ 반드시 사용자에게 브라우저 캡처 스크린샷을 다시 요청한다
  │  리팩토링 후에도 UI가 동일한지 검증 (동일 Figma 기준으로 재채점)
  │
  ├─ 100점 → [Step 6]으로 진행
  └─ <100점 → Architect가 해당 변경을 롤백 후 [Step 5] 재검증
  │
  ▼
[Step 6] 최종 결과 보고
```
...
**모든 검증 단계(Step 3, Step 5)에서 반드시 사용자에게 브라우저 캡처 스크린샷을 요청한다.**
Verifier는 자체적으로 브라우저를 캡처할 수 없으므로, 매번 사용자에게 명시적으로 요청해야 한다.
스크린샷 없이 검증을 진행하거나 건너뛰는 것은 절대 허용되지 않는다.

**검증 시 Figma 재분석 필수**: 검증을 진행할 때는 항상 Figma MCP(`get_design_context`, `get_screenshot`)를 통해 구현 중인 화면의 디자인 정보를 **다시 읽어와 분석한 후** 검증한다. 이전에 읽어둔 정보에 의존하지 않는다.

**잔여 이슈 즉시 수정 필수**: 검증 후 이슈가 하나라도 남아 있다면 — 아무리 작은 이슈라도 — 반드시 즉시 수정한다. 수정 후 다시 검증을 돌려 100점이 될 때까지 반복한다.

### 검증 리포트 필수 출력 규칙 (예외 없음)

**사용자가 "검증 진행해", "검증해줘", "확인해줘" 등 구현 결과 확인을 요청하거나, 스크린샷을 첨부하여 비교를 요청하면 반드시 아래 Verifier Diff 리포트를 출력한다.**

1. Figma MCP `get_screenshot`으로 원본 스크린샷을 확보한다 (비교 기준)
2. 사용자가 첨부한 스크린샷 또는 Playwright 캡처를 구현 스크린샷으로 사용한다
3. 두 스크린샷을 `agents/verifier.md`의 **8개 카테고리 채점 기준**으로 비교한다
4. **반드시 아래 형식의 Diff 리포트를 출력한다** — 비공식적 차이점 나열은 허용하지 않는다:
...
## ❌ 불일치 항목
(각 항목에 Figma 값, 현재 값, 수정 방법, 대상 파일 포함)

## ✅ 일치 확인된 부분
```

5. 불합격 시 불일치 항목을 수정한 뒤 **다시 리포트를 출력**한다
6. 이 규칙은 정식 오케스트레이션 흐름뿐 아니라, **사용자가 비공식적으로 검증을 요청한 경우에도 동일하게 적용**된다
...
## 결과 요약
- 코드 검증 (Step 3) 최종 점수: 100/100 ✅
- 리팩토링 검증 (Step 5) 최종 점수: 100/100 ✅
- 총 반복 라운드: N회
