...
1. **기존 공통 컴포넌트 재사용 우선**: 코드 작성 전 `src/components/common/` 디렉토리를 반드시 확인하여 이미 만들어진 공통 컴포넌트(SortIcon, FilterIcon, InfoIcon, InfoTooltip, ConfirmDialog 등)가 있는지 검토한다
   - 동일하거나 유사한 UI 요소가 공통 컴포넌트로 존재하면 **반드시 재사용**한다
   - 로컬에서 중복 구현하지 않는다
   - 공통 컴포넌트에 없는 경우에만 새로 만들되, 다른 페이지에서도 사용될 가능성이 있으면 `src/components/common/`에 공통 컴포넌트로 생성한다

2. **shadcn/ui 우선**: Figma 요소가 shadcn 컴포넌트에 대응되면 반드시 shadcn을 사용한다
   - `references/shadcn-patterns.md` 참고
   - 매핑되는 컴포넌트가 없는 경우에만 직접 구현

3. **100% 픽셀 퍼펙트**: Figma Reader 분석에 명시된 모든 수치를 정확히 반영한다
...
4. **색상 처리**: 분석 결과의 정확한 hex 값을 사용한다
   - 디자인 토큰이 있으면 CSS 변수로 정의 후 참조
   - 토큰이 없으면 arbitrary value로 직접 지정: `bg-[#515E94]`, `text-[#384047]`

5. **TypeScript 엄격 타입**: 모든 컴포넌트 props에 interface 정의, `any` 금지

6. **에셋 처리**: 분석 결과의 에셋 URL을 직접 사용. placeholder 금지

7. **Figma px → Tailwind 변환표**:
...
- **탭 기반 페이지**: 각 탭의 컴포넌트를 탭 이름의 하위 폴더에 분리 (예: `src/pages/srs-management/matrix-check/`, `src/pages/srs-management/srs-list/`). 탭 간 공유 파일은 페이지 폴더 루트에 둔다
